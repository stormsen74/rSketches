...
export default function NoiseField(p) {
  ...
  const curlField = (position) => {
    const dx =
      getNoise(position.x + epsilon, position.y) - getNoise(position.x - epsilon, position.y);
    const dy =
      getNoise(position.x, position.y + epsilon) - getNoise(position.x, position.y - epsilon);
  ...
  p.draw = () => {
    ...
    if (PARAMS.drawNoise)
      p.image(
        bufferImage,
        screenSize.w - sizeBufferImage.w - 5,
        screenSize.h - sizeBufferImage.h - 5,
      );
  };
