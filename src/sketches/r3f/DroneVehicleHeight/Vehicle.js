...
class Vehicle {
  ...
  seek(vTarget) {
    ...
    if (distanceToTarget < SEEK_RADIUS) {
      const force = mapRange(distanceToTarget, 0, SEEK_RADIUS, 0, 1);
      this.vecDesired.multiplyScalar(force);
    } else {
  ...
  update() {
    ...
    this.location.clamp(new Vector3(-60, -10, -60), new Vector3(60, 10, 60));
