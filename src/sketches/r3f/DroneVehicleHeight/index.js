...
import { Box3, DirectionalLight, MeshStandardMaterial, TextureLoader, Vector2, Vector3 } from 'three'
...
function PhysicLayer() {
  ...
  const updateOnTarget = () => {
    const vPosXZ = new Vector3(targetMesh.current.position.x, vehicle.current.location.y, targetMesh.current.position.z)
  ...
  return (
    <>
        ...
        {droneDebug && (
          <>
            ...
            <mesh>
              ...
              <meshNormalMaterial transparent={true} opacity={0.5} wireframe={true} visible={true} />
            </mesh>
          </>
        )}
        <primitive object={scene} scale={[1.5, 1.5, 1.5]} rotation={[0, Math.PI, 0]} visible={true} />
      </group>
      ...
      <mesh ref={boxMesh} position={[0, 5, 17]}>
        ...
        <meshBasicMaterial color={areaActive ? '#35db0c' : '#e30b50'} transparent={true} opacity={1} wireframe={true} />
      </mesh>
