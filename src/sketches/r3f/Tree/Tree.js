  ...
  spawn: function (branch, extension) {
    ...
    if (extension) {
      ...
      segmentIndex = Math.floor(
        (Math.random() * (this.rootRange.y - this.rootRange.x) + this.rootRange.x) *
...
function TreeBranch(params) {
  ...
  if (from instanceof TreeSegment) {
    ...
    this.position = from.position
      .clone()
      .add(new THREE.Vector3(0, 1, 0).applyMatrix4(from.rotation).setLength(0.05));
...
TreeBranch.prototype = {
  buildTreeSegments: function (radius, radiusSegments, direction, heightSegments) {
    ...
    for (var i = 1; i < heightSegments; i++) {
      ...
      const radius = fromRadius + (toRadius - fromRadius) * ry;
      var d = p1.distanceTo(p0);
      uvOffset += d / uvLength;

      var segment = new TreeSegment(p0, rotation, uvOffset, radius, radiusSegments);
 ...
 * rotation : THREE.Matrix4
 * */
function TreeSegment(position, rotation, uvOffset, radius, radiusSegments) {
