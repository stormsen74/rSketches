...
function Platonic({ position = undefined, type = undefined }) {
  ...
  const platonicMesh = new Mesh(platonicGeometry, material)
  platonicMesh.castShadow = true
  const edges = new EdgesGeometry(platonicGeometry)
  ...
  return (
    <CanvasContainer>
      <ThreeCanvas camera={camera} clearColor={'#0a1e29'}>
        <OrbitControls />
        <Lights />
        {showPlatonics ? <PlatonicSolids /> : <Platonic position={[0, 0, 0]} rotation={[0, 1, 0]} />}
      </ThreeCanvas>
    </CanvasContainer>
  )
}
