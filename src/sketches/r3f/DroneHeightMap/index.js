...
function PhysicLayer({ orbitControls }) {
  ...
  const updateReferenzPosition = () => {
    ...
    if (canvasHeightMap.current) canvasHeightMap.current.updatePosition(vPosXZ)
  }
  ...
  return (
    <>
        ...
        {droneDebug && (
          <>
            ...
            <mesh>
              ...
              <meshNormalMaterial transparent={true} opacity={0.25} wireframe={true} visible={true} />
            </mesh>
          </>
        )}
      ...
      <mesh position={[0, 0, 0]} receiveShadow={true} castShadow={true}>
        <boxBufferGeometry args={[3, 8.5, 3, 5, 5, 5]} />
        <meshStandardMaterial color={'#124451'} transparent={true} opacity={1} wireframe={showHeightMap} />
      </mesh>
      <mesh position={[0, 0, 0]} receiveShadow={true} castShadow={true}>
